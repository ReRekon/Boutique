...
public class OrderController
{
    ...
    public ReturnOrder createOrderByShoppingCart(@PathVariable List<Integer> idList) {
        ...
        {
            list.add(orderServiceImpl.getShoppingItem(id,item));
        }
        ...
        for (ShoppingCartItem item : list) {
           ...
           // System.out.println(item.getProductId());
            product= orderServiceImpl.getState(item.getProductId());
        ...
        //将生成订单加入数据库
        if(orderServiceImpl.insertOrder(order)==0)
        {
        ...
        for (ShoppingCartItem item : list)
        {
            ...
            //获得商品规格
            orderItem.setProductSpecification(orderServiceImpl.findSpecification(item.getProductId(),item.getProductSpecificationId()));
            orderItem.setProductName(orderServiceImpl.getState(item.getProductId()).getName());
    ...
    public String payOrder(Map map)
    {
        ...
        if(map==null)
        {
            ...
            {
                orderServiceImpl.deleteOrderItem(item.getOrderItemId());
            }
        }
        else {
            /*
            ...
            orderServiceImpl.updateOrder(order.getOrderId(),1,new Date());

            for(OrderItem item:list)
            {
                //查询商品库存并减少库存
                Product product= orderServiceImpl.getState(item.getProductId());
                long inventory=product.getInventory();

                inventory=inventory-item.getNumber();
                orderServiceImpl.updateProduct(item.getProductId(),inventory);
            ...
            {
                //如果状态为已经被勾选，则删除
                orderServiceImpl.deleteShoppingCartItem(item.getProductId());
            }
            return "支付成功";
        }
        return "支付失败";
    ...
    public String cancelOrder()
    {
        ...
        //获取当前用户的订单状态
        List<Order> list= orderServiceImpl.queryOrders(id);
        ...
        for(Order item:list)
        {
            ...
            {
                tag=1;
                orderServiceImpl.deleteOrder(item.getOrderId());
                orderServiceImpl.deleteOrderItem(item.getOrderId());
    ...
    public String deleteOrder(@PathVariable int orderId) {
        ...
        for (Order item : list) {
            ...
            if (item.getState() == 1 && item.getOrderId() == orderId) {
                //删除订单更改状态为3,为已删除状态
                orderServiceImpl.updateDeleteOrder(orderId, 3);
                tag=1;
                break;
            }
        }
    ...
    public ReturnOrder findAllOrders()
    {
        ...
        List<Order> orderList= orderServiceImpl.queryOrders(id);
        ...
        for(Order item : orderList){
            List<OrderItem> orderItemList=orderServiceImpl.queryOrderItemByOrderId(item.getOrderId());
            item.setOrderItem(orderItemList);
            for (OrderItem orderItem : orderItemList)
            {
                ProductSpecification productSpecification=orderServiceImpl.findSpecification(orderItem.getProductId(),orderItem.getProductSpecificationId());
